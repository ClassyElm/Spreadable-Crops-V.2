tag @e[tag=csv2tscm,nbt={Age:5999}] add csv2tcnfg

# Remove crop not fully grown tag if crop is actually fully grown
...
# Reset age of area effect cloud where crop is not fully grown
execute as @e[tag=csv2tscm,tag=csv2tcnfg] run data merge entity @s {Age:0}

execute as @e[tag=csv2tscm,nbt={Age:5999}] at @s run function classyelm_crop_spreading:spread/randomize
